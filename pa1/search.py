...
def graphSearch(problem, fringe):
    ...
    fringe.push(start)
    while(1):
        emptysituation = fringe.isEmpty()
        if emptysituation:
            raise Exception("no solution ")
        node = fringe.pop()
        goalsituation = problem.isGoalState(node.getNode())
        if goalsituation:
            return node.getActions()
        nodenotclose = node.getNode() not in closed
        if nodenotclose:
            closed.add(node.getNode())
            index = problem.getSuccessors(node.getNode())
            for (successor,action,stepCost) in index:
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while(True):
        ...
        if goalsituation:
            return node.getActions()
        nodenotclose = node.getNode() not in closed
        if nodenotclose or node.getTotalCost() < closed[node.getNode()]:
